    ...
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <small class="text-muted">Tren pendapatan dan pengeluaran harian</small>
            </div>
            
            <!-- Toggle Period -->
            <div class="btn-group" role="group">
                <button type="button" class="btn btn-primary btn-sm" id="btn-daily" onclick="togglePeriod('daily')">
                    <i class="cil-calendar mr-1"></i> Harian
                </button>
                <button type="button" class="btn btn-outline-secondary btn-sm" id="btn-monthly" onclick="togglePeriod('monthly')">
                    <i class="cil-calendar-check mr-1"></i> Bulanan
                </button>
            </div>
        </div>
    </div>
    ...
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <small class="text-muted">Ringkasan kinerja penjualan berdasarkan periode</small>
            </div>
...
function togglePeriod(period) {
    ...
    if (period === 'daily') {
        ...
        btnMonthly.className = 'btn btn-outline-secondary btn-sm';
        currentChart.data.datasets[0].data = dailyIncome;
        currentChart.data.datasets[1].data = dailyOutcome;
    } else {
        btnDaily.className = 'btn btn-outline-secondary btn-sm';
        btnMonthly.className = 'btn btn-primary btn-sm';
        currentChart.data.datasets[0].data = monthlyIncome;
        currentChart.data.datasets[1].data = monthlyOutcome;
    }

    currentChart.update('active'); // smooth transition
} else {
        btnDaily.className = 'btn btn-outline-secondary btn-sm';
        btnMonthly.className = 'btn btn-primary btn-sm';
    }
    ...
    const newData = period === 'daily' ? dailyData : monthlyData;

    // Custom smooth transition animation
    currentChart.data.labels = newData.labels;
    currentChart.data.datasets[0].data = newData.datasets[0].data;
    currentChart.data.datasets[1].data = newData.datasets[1].data;

    // Update with custom animation config
    currentChart.update({
        duration: 800,
                ...
                from: (ctx) => {
                    if (ctx.type === 'data') {
                        if (ctx.mode === 'default' && !ctx.dropped) {
                            ctx.dropped = true;
                            return ctx.chart.scales.x.getPixelForValue(ctx.chart.scales.x.max);
                        }
                    }
                }
            },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // helper biar dataset panjangnya konsisten
    ...
    const fullLabels = Array.from({ length: 31 }, (_, i) => i + 1);

    const dailyIncome   = normalizeData(chartData.dailyIncome, 31);
    const dailyOutcome  = normalizeData(chartData.dailyOutcome, 31);
    const monthlyIncome = normalizeData(chartData.monthlyIncome, 31);
    const monthlyOutcome= normalizeData(chartData.monthlyOutcome, 31);
