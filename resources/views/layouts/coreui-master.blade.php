...
<body>
    ...
    <script>
                $(document).ready(function() {
            // Fungsi untuk menginisialisasi atau me-refresh tooltip di tabel
            function initializeTableTooltips() {
                // Hancurkan tooltip yang sudah ada untuk menghindari duplikasi
                $('[data-bs-toggle="tooltip"]').tooltip('dispose');

                // Inisialisasi semua tooltip yang ada di halaman (termasuk di tabel)
                // dengan konfigurasi yang kita inginkan
                var tooltipTriggerList = [].slice.call(document.querySelectorAll('[data-bs-toggle="tooltip"]'));
                var tooltipList = tooltipTriggerList.map(function (tooltipTriggerEl) {
                    return new bootstrap.Tooltip(tooltipTriggerEl, {
                        container: 'body', // Penting agar tooltip tidak terpotong
                        trigger: 'hover',
                        customClass: 'custom-tooltip', // Class CSS yang kita buat di Langkah 1
                        delay: { "show": 300, "hide": 100 }
                    });
                });
            }
            ...
            // Jika kamu menggunakan DataTables, tambahkan event listener ini
            // untuk memastikan tooltip tetap berfungsi setelah paginasi atau search
            if ($.fn.dataTable) {
                // Ganti '#namaTabelAnda' dengan ID tabelmu yang sebenarnya, contoh: '#table-produk'
                $('table.table').on('draw.dt', function () {
                    console.log('Tabel digambar ulang, tooltip diinisialisasi ulang.');
                    initializeTableTooltips();
                });
            }
        });
